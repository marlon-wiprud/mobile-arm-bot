...
from control_msgs.msg import FollowJointTrajectoryAction, FollowJointTrajectoryGoal  # Controller messageos
...
# func input is array of floats corresponding to the values that the arm should use to move
...
def move_robot_arm(joint_values):
    ...
    arm_goal.trajectory.joint_names = ['arm_base_joint', 'shoulder_joint',
...
def move_robot_base(twist_values):
    pub = rospy.Publisher('robot_base_velocity_controller/cmd_vel', Twist, queue_size=1)
    rate = rospy.Rate(1)

    while not rospy.is_shutdown():
        print('publishing values...', twist_values)
        pub.publish(twist_values)
        rate.sleep()
...
if __name__ == '__main__':
    ...
    try:
        ...
        twist_values = Twist()
        twist_values.linear.x = 1
        twist_values.angular.z = 0
        twist_values.linear.y = 0

        print('moving robot base...')
        move_robot_base(twist_values)
